...
import { AppError, ForbiddenError, NotFoundError, apiFailure, apiSuccess, parseJson, requestIdFrom } from "@/lib/server/api";
...
export async function GET(request: Request, { params }: { params: Promise<{ saleId: string }> }) {
  ...
  try {
    ...
    if (!access.isSuperAdmin && !access.permissions.has("sales.view") && !access.permissions.has("sales.create")) throw new ForbiddenError("You do not have permission to view this sale.");
