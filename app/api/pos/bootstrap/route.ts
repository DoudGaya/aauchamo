...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [products, customers, paymentMethods, businessUnits, agents] = await Promise.all([
      ...
      db.customer.findMany({ where: { companyId: access.companyId, status: "ACTIVE", ...(access.companyWide && !access.stationIds.size ? {} : { homeStationId: { in: [...access.stationIds] } }) }, select: { id: true, customerNumber: true, displayName: true, primaryPhone: true }, orderBy: { displayName: "asc" }, take: 500 }),
      db.paymentMethod.findMany({ where: { companyId: access.companyId, isActive: true }, orderBy: { displayOrder: "asc" } }),
      db.stationBusinessUnit.findMany({ where: { stationId, businessUnit: { isActive: true, ...(!access.businessUnitIds.size ? {} : { id: { in: [...access.businessUnitIds] } }) } }, include: { businessUnit: true }, orderBy: { businessUnit: { name: "asc" } } }),
      db.agent.findMany({ where: { companyId: access.companyId, status: "ACTIVE", ...(access.companyWide && !access.stationIds.size ? {} : { homeStationId: { in: [...access.stationIds] } }) }, include: { wallet: true }, orderBy: { name: "asc" } }),
    ]);
